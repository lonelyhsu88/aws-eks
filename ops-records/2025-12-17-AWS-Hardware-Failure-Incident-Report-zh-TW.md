...
2025年12月17日，AWS 在 ap-east-1（香港）區域發生硬體可用性問題，影響我們的正式環境 EKS 叢集 `gemini-game-prd`。該事故由 AWS 基礎設施和我們的 Auto Scaling Groups 自動偵測並解決，無需人工介入，證明了我們高可用性架構的有效性。

**關鍵指標**：
- **事故持續時間**：32 分 48 秒（14:23-14:56 HKT）
...
| 14:41:30 | 🔄 節點啟動 | gemini-hash：2 個實例啟動 | 臨時恢復 | 恢復中 |
| 14:41:36 | 🔄 節點啟動 | gemini-bg：臨時節點啟動 | Pod 遷移開始 | 恢復中 |
| 14:41:52 - 14:54:15 | ⚠️ 啟動失敗 | gemini-hash：**連續 10 次失敗** | ap-east-1b 容量耗盡 | 嚴重 |
| 14:51:41 | 🔄 節點終止 | gemini-bg：臨時節點終止 | 第二次遷移觸發 | 恢復中 |
| **14:56:00** | ✅ **AWS 解決** | 硬體問題解決 | 基礎設施穩定 | 已解決 |
| 14:56:27 | ✅ 節點成功 | gemini-hash：在 ap-east-1a 啟動 | AZ 容錯移轉成功 | 恢復中 |
...
1. **AWS 偵測到解決**：32 分 48 秒
2. **首次啟動失敗到成功**：約 23 分鐘
3. **總啟動失敗次數**：**15 次嘗試**（前所未有）
4. **應用程式恢復時間**：14:23 - 15:12（約 49 分鐘）
5. **通知延遲**：17 分鐘（AWS 郵件 vs. 實際開始）
...
- **14:40-15:00**：監控和評估
- **15:00-15:30**：事故後容量調整（gemini-hash Max: 3→5）
- **15:30-16:00**：影響分析和文件記錄

**關鍵觀察**：系統在需要人工介入前已完全恢復，驗證了自動化策略。
...
- 零人工介入需求
- 無資料遺失
- 有效的 Auto Scaling 和 Kubernetes 自我修復
- 全程維持監控和可觀測性
...
1. **自動化有效**：我們在 Auto Scaling Groups 和 Kubernetes 自我修復上的投資獲得了回報。事故在沒有人工介入的情況下自動解決。

2. **容量規劃很重要**：當 15 次啟動嘗試失敗時，gemini-hash 節點群組的 Max 容量不足（3）變得明顯。緊急增加至 Max=5 是必要且有效的。

3. **單點故障有風險**：以單副本運行的服務（ArgoCD、閘道、Redis）造成不必要的脆弱性。ArgoCD 的情況特別嚴重，因為它是單一節點託管所有 7 個 pod。

4. **實例類型多樣性至關重要**：依賴單一實例類型（c5a.xlarge）使我們容易受到容量短缺的影響。這將透過混合實例策略來解決。

5. **監控和警報需要改善**：17 分鐘的通知延遲意味著我們在事故已經進行一段時間後才得知。
